...
var SimpleCalendar = function () {
  ...
  _createClass(SimpleCalendar, [{
    ...
    value: function create() {
      ...
      //actions
      header.innerHTML = header.innerHTML + '<div class="sc-actions">' + '<div class="sc-yleft" style="cursor: pointer;margin-right:5px;">' + '&lsaquo;</div>' + '<select class="sc-select-year" name="">' + '</select>' + '<div class="sc-yright" style="cursor: pointer;margin-left:5px;">' + '&rsaquo;</div>' + '</div>';
      header.innerHTML = header.innerHTML + '<div class="sc-actions">' + '<div class="sc-mleft" style="cursor: pointer;margin-right:5px;">' + '&lsaquo;</div>' + '<select class="sc-select-month" name="">' + '</select>' + '<div class="sc-mright" style="cursor: pointer;margin-left:5px;">' + '&rsaquo;</div>' + '</div>';
      // header.innerHTML = header.innerHTML + '<div class="sc-actions" style="cursor: pointer;"><span class="sc-return-today ">返回今天</span></div>';
...
//      self.setInterval('SimpleCalendar.timeupdate()', 200);
  ...
  //刷新日历
        ...
        if (data) {
          daysElement.forEach(function (v, i) {
            ...
            if (day == 1) currentmonth++;
        		// console.info(year + '-' + currentmonth + '-' + day)
            if (data[year + '-' + currentmonth + '-' + day]) {
            	// console.info("1111111")
              v.classList.add('sc-mark');
              //日历任务数字显示
              var mark_data = data[year + '-' + currentmonth + '-' + day].length;
              v.innerHTML = v.innerHTML + '<div class="task_count"><span class="badge badge-important count">'+mark_data+'</span></div>';
            } else {
              v.classList.remove('sc-mark');
              v.title = '';
              //删除日历任务显示
              var task_count =  v.querySelector('.task_count');
                if(task_count != null) {
                    v.removeChild(v.querySelector('.task_count'))
                }
            }
              //日历添加点击事件
              //v.attachEvent('onclick', function (){calendarDayClick();});
          });
        }
      }
    }
    ...
    //刷新节日数据
	    ...
	    key: 'updateDelay',
	    value: function updateDelay(year, month) {
	      var options = this._options;
	        var daysElement = this.arrayfrom(this.container.querySelectorAll('.sc-item'));
	        var currentmonth = month - 1;

	        var delayList = options.delayCount;
	        if (delayList) {
	            daysElement.forEach(function (v, i) {
	                ...
	                if (delayList[year + '-' + currentmonth + '-' + day]) {
	                    var delayCount = delayList[year + '-' + currentmonth + '-' + day];
	                    if(delayCount > 0)
	                    {
	                    	 v.classList.add('sc-delay');
	                    }
	                }
	              });
	        }
    ...
    value: function updateEvent() {
      ...
      yearadd.onclick = function () {
        ...
        //统计每个日期的事件的个数,在所调用的页面中定义
        itemCount(this.tmonth,currentyear);
      ...
      monthadd.onclick = function () {
        ...
        //itemCount(currentmonth,currentyear);
      ...
      returntoday.onclick = function () {
        ...
        itemCount(calendar.tmonth,calendar.tyear);
    ...
    key: 'addMark',
    value: function addMark(day, info,month,year) {
        if(info instanceof Array){
            for(var i in info)
            {
                this._options.mark[info[i].day] = info[i].value;
                this._options.delayCount[info[i].day] = info[i].delayCount;
            }
        }else{
    ...
    //添加延期样式
	    ...
	    key: 'addDelay',
	    value: function addDelay(day, info,month,year) {
	        if(info instanceof Array){
	            for(var i in info)
	            {
	                this._options.delayCount[info[i].day] = info[i].delayCount;
	            ...
	            this._options.delayCount[day] = info;
	        }
